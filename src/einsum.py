...
def parse_op_labels(labelstr, operand):
    ...
    # Check shape
    ndims = len(operand) # Note, in Paddle a tensor rank is always nonzero
    assert ndims > 0
...
def plan_reduce(plan, op, reduce_dims, keepdim):
    ...
    f = lambda var, dims: ops.sum(var, dims, keepdim=keepdim)
...
def plan_scalar_prod(plan, op1, op2):
    ...
    f = lambda var1, var2: ops.sum(var1) * var2
...
def plan_matmul(plan, g_view, op1, op2, g_op_masks, g_shape, I, J1, J2, K):
    ...
    if any(i != dim for i, dim in enumerate(perm1)):
        # print(f'perm1: {perm1}')
        step = ops.transpose, [var1], var1, tuple(perm1)
        plan.add_step(step)

    if any(i != dim for i, dim in enumerate(perm2)):
        # print(f'perm2: {perm2}')
        step = ops.transpose, [var2], var2, tuple(perm2)
        plan.add_step(step)
    ...
    if not K:
        ...
        if J2:
            ...
            fill = tuple(range(fill_start, fill_end))
        ...
        if J1:
            ...
            plan.add_step(step)
        # make broadcast
        step = ops.multiply, [var1, var2], var2
        plan.add_step(step)
    ...
    # In case I... and J... are empty, do the vector-vector version of matmul
        ...
        if len(K) > 1:
            for var in var1, var2:
                step = ops.reshape, [var], var, [K1_size]
                plan.add_step(step)
    ...
    # matrix-vector or matrix-matrix multiplies, depending on the operands' shapes.
        ...
        step = ops.reshape, [var1], var1, merged_shape1
        plan.add_step(step)
        step = ops.reshape, [var2], var2, merged_shape2
        plan.add_step(step)
    ...
    if result_shape:
        step = ops.reshape, [var2], var2, result_shape
        plan.add_step(step)
...
def plan_broadcast(plan, operands, nop_axes):
    ...
    for i, op_axes in zip(range(nop), nop_axes):
        ...
        var = varnames[i]
        if perm:
            step = ops.transpose, [var], var, perm
            plan.add_step(step)
        if fill:
...
def plan_einsum(operands, g_view, g_shape, g_op_masks, g_count, n_bcast):
    ...
    if any(ax != dim for ax, dim in enumerate(view[:nout])):
        ...
        varname = f'op{nop-1}'
        step = ops.transpose, [varname], varname, tuple(perm)
        plan.add_step(step)
        dim = 0
    ...
    if squeeze_dims:
        ...
        varname = f'op{nop-1}'
        step = ops.squeeze, [varname], varname, tuple(squeeze_dims)
        plan.add_step(step)

    return plan

@ops.constexpr
def part_equation(equation):
...
@ops.constexpr
def get_n_bcast_dims(nop_labels):
    return max(map(lambda s: s.count('.'), nop_labels))
...
def einsum(equation, *operands):
    ...
    lhs, rhs = part_equation(equation)
    ...
    nop_labels = parse_labels(lhs, [operand.shape for operand in operands])
    ...
    # e.g. 1 for ['ij', 'i.', '.k']
    n_bcast_dims = get_n_bcast_dims(nop_labels)
